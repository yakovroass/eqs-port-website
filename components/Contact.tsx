...
function DownloadRow({ className = "" }: { className?: string }) {
  ...
  const btn =
    "flex flex-1 min-w-0 items-center justify-center gap-0.5 sm:gap-2 py-2.5 px-0.5 sm:px-3 rounded-xl font-semibold whitespace-nowrap text-[10px] min-[400px]:text-[11px] sm:text-sm leading-tight transition-colors border border-cyan-400/40 text-cyan-300 hover:bg-cyan-500/15";

  return (
    <div className={`flex flex-nowrap gap-1.5 sm:gap-3 w-full max-w-xl mx-auto ${className}`}>
      <a href="/pitch-deck.pdf" download className={btn}>
      ...
      <a href="/EQS.PORT.1P.pdf" download className={btn}>
        <svg className="w-3.5 h-3.5 sm:w-4 sm:h-4 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5} aria-hidden>
          <path strokeLinecap="round" strokeLinejoin="round" d="M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5M16.5 12L12 16.5m0 0L7.5 12m4.5 4.5V3" />
        </svg>
            ...
            <div className="flex flex-col gap-4">
              <div className="glass-card rounded-2xl p-6 sm:p-8">
                ...
                <a
                  ...
                  className="mt-6 w-full flex items-center justify-center gap-2 py-3.5 px-4 rounded-xl font-semibold bg-cyan-500/20 border border-cyan-400/50 text-cyan-300 hover:bg-cyan-500/30 hover:text-white transition-colors"
                >
