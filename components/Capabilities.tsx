...
const mockupMap: Record<CapKey, "search" | "analysis" | "revalue" | "crm" | "messaging" | "logistics" | "permits" | "spareparts" | "map" | "quotes"> = {
  search: "search",
  analysis: "analysis",
  offers: "revalue",
  crm: "crm",
        ...
        <ScrollReveal>
          <div className="text-center mb-4">
            <span className="text-accent font-mono text-base sm:text-lg tracking-widest uppercase">
              {tx(t.capabilities.label, lang)}
            </span>
          </div>
          <h2 className="text-3xl md:text-5xl font-bold text-center mb-6">
            {tx(t.capabilities.headline, lang)}{" "}
            <span className="gradient-text">{tx(t.capabilities.headlineAccent, lang)}</span>
          </h2>
          <p className="text-gray-400 text-center max-w-3xl mx-auto mb-16 text-lg leading-relaxed">
            {tx(t.capabilities.sub, lang)}
          </p>
        </ScrollReveal>
              ...
              <ScrollReveal key={key} delay={i * 0.05}>
                ...
                >
                  <div className={`w-10 h-10 rounded-lg flex items-center justify-center mb-3 transition-colors ${
                    isActive ? "bg-accent/20 text-accent" : "bg-dark-600/60 text-gray-400"
                  }`}>
                    {capIcons[key]}
                  </div>
                ...
                <div className="relative">
                  ...
                  <div className="relative bg-dark-700/30 rounded-2xl p-4 border border-gray-700/30">
                    <div className="mb-3 flex items-center justify-between">
                      ...
                      <button onClick={() => setActive(null)} className="text-gray-500 hover:text-white transition-colors p-1">
